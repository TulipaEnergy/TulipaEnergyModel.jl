...
| Transport Capacity Constraints (upper bounds)  | flow                           | availability     | power           | if it connects two consumers then max(consumer a, consumer b), otherwise its own         | mean                |
| Transport Capacity Constraints (lower bounds)  | flow                           | availability     | power           | if it connects two consumers then max(consumer a, consumer b), otherwise its own         | mean                |
