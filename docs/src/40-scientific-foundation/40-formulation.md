...
| $p^{\text{init units}}_{a,y}$                         | $\mathbb{R}_{+}$         | $a \in \mathcal{A}$, $y \in \mathcal{Y}$                                                           | Number of existing units of asset $a$ at year $y$, given by the user; the model does not decommission them                          | [units]                    |
| $p^{\text{init units}}_{a,y,v}$                       | $\mathbb{R}_{+}$         | $ (a,y,v) \in \mathcal{D}^{\text{compact profiles}}$                                               | Number of existing units of asset $a$ at year $y$ commissioned in year $v$, given by the user; the model does not decommission them | [units]                    |
...
| $v^{\text{decom aggregated}}_{a,y,v}$                 | $\mathbb{Z}_{+}$ | $a \in \mathcal{A}^{\text{aggregated}}$, $y \in \mathcal{Y}$, $v \in \{\mathcal{Y}^{\text{i}}_a: y - p^{\text{technical lifetime}}_{a} + 1 \le v < y\}$                                                           | Number of units of asset $a$ that uses aggregated method, invested in year $v$ and decommissioned at year $y$                                                                          | [units]    |
| $v^{\text{decom compact profiles}}_{a,y,v}$           | $\mathbb{Z}_{+}$ | $(a,y,v) \in \mathcal{D}^{\text{compact profiles}}$, $v \in \{\mathcal{Y}^{\text{i}}_a: v < y\}$                                                                                                                  | Number of units of asset $a$ that uses compact profiles method, invested in year $v$ and decommissioned at year $y$                                                                    | [units]    |
...
| $v^{\text{decom energy aggregated}}_{a,y,v}$          | $\mathbb{Z}_{+}$ | $a \in \mathcal{A}^{\text{i}}_y \cap \mathcal{A}^{\text{se}}_y$, $y \in \mathcal{Y}$, $v \in \{\mathcal{Y}^{\text{i}}_a: y - p^{\text{technical lifetime}}_{a} + 1 \le v < y\}$                                   | Number of units of the energy component of the storage asset $a$ that uses energy method, invested in year $v$ and decommissioned at year $y$                                          | [units]    |
...
| $v^{\text{decom aggregated}}_{f,y,v}$                 | $\mathbb{Z}_{+}$ | $f \in \mathcal{F}^{\text{ti}}_y$, $y \in \mathcal{Y}$, $v \in \{\mathcal{Y}^{\text{i}}_f: y - p^{\text{technical lifetime}}_{f} + 1 \le v < y\}$                                                                 | Number of units of capacity increment of transport flow $f$, invested in year $v$ and decommissioned at year $y$                                                                       | [units]    |
...
```math
\begin{aligned}
    v^{\text{available units aggregated}}_{a,y} & = p^{\text{initial units}}_{a,y} + \sum_{v \in \{\mathcal{Y}^\text{i}_a: y - p^{\text{technical lifetime}}_{a} + 1  \le v \le y \}} \left( v^{\text{inv}}_{a,v} - \sum_{i \in \{\mathcal{Y}: v < i \le y \}} v^{\text{decom aggregated}}_{a,i,v} \right) \\
 ...
 \\
    ...
    v^{\text{available energy units aggregated}}_{a,y} & = p^{\text{initial storage units}}_{a,y} + \sum_{v \in \{\mathcal{Y}^\text{i}_a: y - p^{\text{technical lifetime}}_{a} + 1  \le v \le y \}} \left( v^{\text{inv energy}}_{a,v} - \sum_{i \in \{\mathcal{Y}: v < i \le y \}} v^{\text{decom energy aggregated}}_{a,i,v} \right) \\
    & \forall a \in \mathcal{A}^{\text{se}}_y, \forall y \in \mathcal{Y} \\
    v^{\text{available export units aggregated}}_{f,y} & = p^{\text{initial export units}}_{f,y} + \sum_{v \in \{\mathcal{Y}^\text{i}_f: y - p^{\text{technical lifetime}}_{f} + 1  \le v \le y \}} \left( v^{\text{inv}}_{f,v} - \sum_{i \in \{\mathcal{Y}: v < i \le y \}} v^{\text{decom aggregated}}_{f,i,v} \right) \\
    & \forall f \in \mathcal{F}^{\text{t}}_y, \forall y \in \mathcal{Y} \\
    v^{\text{available import units aggregated}}_{f,y} & = p^{\text{initial import units}}_{f,y} + \sum_{v \in \{\mathcal{Y}^\text{i}_f: y - p^{\text{technical lifetime}}_{f} + 1  \le v \le y \}} \left( v^{\text{inv}}_{f,v} - \sum_{i \in \{\mathcal{Y}: v < i \le y \}} v^{\text{decom aggregated}}_{f,i,v} \right) \\
    & \forall f \in \mathcal{F}^{\text{t}}_y, \forall y \in \mathcal{Y} \\
\end{aligned}
```

The initial units ($p^{\text{initial units}}_{a,y}$, $p^{\text{initial units}}_{a,y,v}$, $p^{\text{initial storage units}}_{a,y}$, $p^{\text{initial export units}}_{f,y}$, and $p^{\text{initial import units}}_{f,y}$) are the existing units that the user declares for each milestone year.
They are data, not decisions: the user is responsible for which existing units are still available at each milestone year, and the model never decommissions them.
The model only decommissions units that it invested in.
For that, the decommission variables carry the commission year $v$ of the decommissioned units, and a decommission is only subtracted while those units could still exist, i.e., within the technical lifetime of their vintage.
...
In the compact methods, $v^{\text{available units compact}}_{a,y,v}$ only exists for the vintages available in year $y$, so the lifetime is already taken into account by the set $\mathcal{D}^{\text{compact profiles}}$.
...
The model only decommissions units that it invested in, so the decommissions of each investment vintage are bounded by the investment of that vintage.
The existing units are given by the user and are not part of these constraints.
...
#### Upper Limit for Decommissioned Units Compact method
...
```math
\begin{aligned}
    v^{\text{inv}}_{a,v} - \sum_{i \in \{\mathcal{Y}: v < i \} | (a,i,v) \in \mathcal{D}^{\text{compact profiles}}} v^{\text{decom compact profiles}}_{a,i,v} \ge 0 \quad  \forall a \in \mathcal{A}^{\text{compact profiles}} \cup \mathcal{A}^{\text{compact efficiencies}}, \forall v \in \mathcal{Y}^{\text{i}}_a
\end{aligned}
```
...
Each inequality is included only when its corresponding `asset_milestone` field is defined. The available-unit expression uses initial units (given by the user), active investments, and decommissions of invested units; for compact assets it sums all active vintages. These limits apply to assets only.
