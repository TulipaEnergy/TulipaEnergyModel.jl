...
- DuckDB / SQL
  ...
  - For large write paths, prefer `DuckDB.Appender` or `DuckDB.register_table` together with `UPDATE ... FROM` over repeated single-row inserts or updates.
  - Prefer parameter binding for scalar values via `DuckDB.execute(..., [value])` when possible. Interpolate only validated identifiers or generated SQL fragments; when interpolating SQL string literals, quote them with `TulipaIO.FmtSQL.fmt_quote`.
