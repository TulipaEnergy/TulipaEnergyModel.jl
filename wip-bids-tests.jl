using DataFrames, TulipaIO, TulipaEnergyModel, DuckDB, TulipaClustering
using TulipaBuilder
using JuMP
...
function create_problem()
    ...
    add_asset!(tulipa, "Generator", :producer; capacity = 1000.0, initial_units = 1.0)
    add_asset!(tulipa, "Bid Manager", :consumer; peak_demand = 0.0) # no demand
    add_flow!(tulipa, "Generator", "Bid Manager"; operational_cost = 10.0)
...
debug(str) = println(DataFrame(sql(str)))
...
@assert energy_problem.objective_value ≈ 0.0
...
from_bids_insert_into("flow_milestone", "asset AS from_asset, asset AS to_asset, $milestone_year")
from_bids_insert_into("flow_commission", "asset AS from_asset, asset AS to_asset, $commission_year")
...
println(
    DataFrame(
        sql("FROM var_flow WHERE to_asset LIKE 'bid%' AND time_block_start BETWEEN 6 AND 10"),
    ),
)

expected_solution = 10 * (10.0 - 500.0)

@assert energy_problem.solved
@info energy_problem.objective_value, expected_solution
