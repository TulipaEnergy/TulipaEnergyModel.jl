...
function fix_storage_levels!(
    ...
    bench_pairs = [(bench_vars[i], parse_rep_period_name(JuMP.name(bench_vars[i])))
                   for i in eachindex(bench_vars)]

    red_pairs = [(red_vars[i], parse_over_clustered_name(JuMP.name(red_vars[i])))
                 for i in eachindex(red_vars)]
...
function plot_storage_behavior(
    ...
    hourly_filtered_asset = filter(
        row ->
            row.rep_period == scenario,
        hourly_filtered_asset,
    )
    ...
    for (i, rp) in enumerate(representative_periods)
        ...
        for g in groupby(results_with_options, :base_name)
            ...
            path = joinpath(
                tables_path, "fixed", name_rp, "Gurobi", "var_storage_level_rep_period.csv"
            )

            reduced_storage_levels = CSV.read(path, DataFrame)

            reduced_filtered_asset = filter(
                row -> row.asset == asset_to_filter,
                reduced_storage_levels,
            )
            reduced_filtered_asset = filter(
                row ->
                    row.rep_period == scenario,
