@testset "Test missing defaults and populate_with_defaults!" begin
    ...
    @testset "Test that Tiny has all default data and calling populate won't break it" begin
        ...
        asset_capacity = Dict(
            row.asset => row.capacity for row in DuckDB.query(
                connection,
                "SELECT asset.asset, asset.capacity FROM input_asset as asset",
            )
        )
        ...
        for row in
            DuckDB.query(connection, "SELECT asset.asset, asset.capacity FROM input_asset as asset")
    ...
    @testset "Test 1 missing column in Tiny" begin
        ...
        for row in
            DuckDB.query(connection, "SELECT asset.asset, asset.capacity FROM input_asset as asset")
            @test row.capacity == TulipaEnergyModel.schema["input_asset"]["capacity"]["default"]
        end
    ...
    @testset "Test that missing column cannot be require" begin
        ...
        DuckDB.query(
            connection,
            "ALTER TABLE input_asset_milestone
