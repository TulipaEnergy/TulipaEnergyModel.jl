...
from
    resolution_asset_rep_period as atr
    left join input_asset as asset on asset.asset = atr.asset
where
...
with
    filtered_assets as (
        ...
        from
            ...
            left join input_asset as asset on attr.asset = asset.asset
        where
            asset.type = 'storage'
...
from
    input_flow_milestone as flow_milestone
    left join input_flow as flow on flow.from_asset = flow_milestone.from_asset
    and flow.to_asset = flow_milestone.to_asset
    left join input_flow_commission as flow_commission on flow_commission.from_asset = flow_milestone.from_asset
...
from
    input_asset_milestone as asset_milestone
    left join input_asset as asset on asset.asset = asset_milestone.asset
    left join input_asset_commission as asset_commission on asset_commission.asset = asset_milestone.asset
...
select
    ...
    asset.investment_integer,
from
    input_asset_both as asset_both
    left join input_asset as asset on asset.asset = asset_both.asset
where
    asset_both.decommissionable
;
...
select
    ...
    flow.investment_integer,
from
    input_flow_both as flow_both
    left join input_flow as flow on flow.from_asset = flow_both.from_asset
