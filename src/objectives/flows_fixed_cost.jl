function _add_flows_fixed_cost!(connection, model, expressions, objective_expr, lambda)
    expr_available_flow_units_aggregated = expressions[:available_flow_units_aggregated]
    ...
    indices = DuckDB.query(
        ...
        "SELECT
            ...
            obj.weight_for_operation_discounts
                ...
                * obj.capacity
                AS cost,
        FROM expr_available_flow_units_aggregated AS expr
    ...
    @expression(
        ...
        sum(
            ...
            (row, avail_export_unit, avail_import_unit) in zip(
                indices,
                expr_available_flow_units_aggregated.expressions[:export],
                expr_available_flow_units_aggregated.expressions[:import],
            )
        )
    )
    _add_to_objective!(
