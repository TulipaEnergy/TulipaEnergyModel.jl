...
"""
    df_flows, df_constraints_lowest, df_constraints_highest = construct_dataframes(
        graph,
...
function construct_dataframes(graph, representative_periods, constraints_partitions)
    ...
    return df_flows, df_constraints_lowest, df_constraints_highest
end
...
function create_model(
    ...
    df_flows, df_constraints_lowest, df_constraints_highest =
        construct_dataframes(graph, representative_periods, constraints_partitions)
    df_storage_level =
        rename(filter(row -> row.asset ∈ As, df_constraints_lowest), :index => :cons_index)
    ...
    for ((a, rp), sub_df) ∈ pairs(df_storage_level_grouped)
        ...
        model[Symbol("storage_balance_$(a)_$(rp)")] = [
            @constraint(
                ...
                (k > 1 ? storage_level[row.index-1] : graph[a].initial_storage_level) +
    ...
    assets_profile_times_capacity =
        model[:assets_profile_times_capacity] = [
            ...
            ) for row in eachrow(df_constraints_highest)
        ]
