...
function calculate_salvage_value(
    ...
    salvage_value_set = Dict(
        (year, asset) => end_of_horizon+1:year+economic_lifetime[asset]-1 for year in years for
        asset in investable_assets[year] if year + economic_lifetime[asset] - 1 ≥ end_of_horizon + 1
    )
