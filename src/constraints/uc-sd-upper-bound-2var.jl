export add_shut_down_upper_bound_2var_constraints!

"""
    add_shut_down_upper_bound_2var_constraints!(model, constraints)

Adds the 2var shut down constraints to the model.
"""
function add_shut_down_upper_bound_2var_constraints!(
    connection,
    model,
    variables,
    expressions,
    ...
    let table_name = :shut_down_upper_bound_simple_investment_2var, cons = constraints[table_name]
        expr_avail_simple_method =
            expressions[:available_asset_units_simple_method].expressions[:assets]

        indices = _append_available_units_start_up_simple_method_2var(connection, table_name)
        ...
        attach_constraint!(
            ...
            [
                ...
                # @constraint(
                #     model,
                #     start_up_vars[row.start_up_id] <= expr_avail_simple_method[row.avail_id],
                #     base_name = "$table_name[$(row.asset),$(row.year),$(row.rep_period),$(row.time_block_start):$(row.time_block_end)]"
                # )
                else
                    @constraint(
                        model,
                        start_up_vars[row.start_up_id] <=
                        expr_avail_simple_method[row.avail_id] - units_on_vars[row.units_on_id-1],
                        base_name = "$table_name[$(row.asset),$(row.year),$(row.rep_period),$(row.time_block_start):$(row.time_block_end)]"
                    )
...
function _append_available_units_start_up_simple_method_2var(connection, table_name)
    return DuckDB.query(
        connection,
        "SELECT
            cons.*,
