...
function add_transport_constraints!(model, graph, df_flows, flow, Ft, flows_investment)
    ...
    upper_bound_transport_flow = [
        if graph[row.from_asset, row.to_asset].investable
            @expression(
                model,
                profile_aggregation(
                    Statistics.mean,
                    graph[row.from_asset, row.to_asset].rep_periods_profiles,
                    (:availability, row.rep_period),
                    row.timesteps_block,
                    1.0,
                ) * (
                    graph[row.from_asset, row.to_asset].initial_export_capacity +
                    graph[row.from_asset, row.to_asset].capacity *
                    flows_investment[(row.from_asset, row.to_asset)]
                )
            )
        else
            @expression(
        ...
        if graph[row.from_asset, row.to_asset].investable
            @expression(
                ...
                profile_aggregation(
                    ...
                    graph[row.from_asset, row.to_asset].initial_import_capacity +
                    graph[row.from_asset, row.to_asset].capacity *
                    flows_investment[(row.from_asset, row.to_asset)]
                )
            )
    ...
    df = filter([:from_asset, :to_asset] => (from, to) -> (from, to) ∈ Ft, df_flows; view = true)
    ...
    model[:max_transport_flow_limit] = [
        @constraint(
            ...
            base_name = "max_transport_flow_limit[($(row.from_asset),$(row.to_asset)),$(row.rep_period),$(row.timesteps_block)]"
    ...
    model[:min_transport_flow_limit] = [
        @constraint(
            ...
            base_name = "min_transport_flow_limit[($(row.from_asset),$(row.to_asset)),$(row.rep_period),$(row.timesteps_block)]"
        ) for row in eachrow(df)
    ]
end
